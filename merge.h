...
>
Container_t<InternalContainer, T>
merge_arrays(Container_t<ExternalContainer, Container_t<InternalContainer, T>> const &arrays,
             Comp comp = Comp()) {
...
#pragma omp single nowait
		{
			std::size_t const merging_pass_number{
					static_cast<std::size_t>(std::ceil(std::log2(block_separator_number_after_first_round)))};
			// O(log2(k)) iterations
